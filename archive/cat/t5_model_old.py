...
class T5Stack(T5PreTrainedModel):
    def __init__(self, config, embed_tokens=None, prefix_length=0, prefix_set_number=0, enable_layer_wise_prefix=True):
        ...
        if self.use_input_prefix_embeds and not self.is_decoder:
            if self.enable_layer_wise_prefix:
                self.prefix_embedding = nn.Embedding(prefix_set_number,
                                                     prefix_length * config.d_model * config.num_layers)
            else:
    ...
    def update_prefix_embedding(self, prefix_set_number):
        ...
        self.prefix_embedding = nn.Embedding(prefix_set_number,
                                             self.prefix_length * self.config.d_model * self.config.num_layers)
    ...
    def forward(
        ...
        if self.use_input_prefix_embeds and not self.is_decoder:
            ...
            if self.enable_layer_wise_prefix:
                prefix_embs = self.prefix_embedding(task_ids).view(inputs_embeds.size(0), self.config.num_layers, -1,
                                                                   self.config.d_model)
            ...
            prefix_mask = torch.ones((attention_mask.size(0), self.prefix_length), dtype=attention_mask.dtype).to(
        ...
        if self.use_input_prefix_embeds and self.is_decoder:
            prefix_mask = torch.ones((attention_mask.size(0), self.prefix_length),
                                     dtype=encoder_attention_mask.dtype).to(encoder_attention_mask.device)
        ...
        for i, (layer_module, past_key_value) in enumerate(zip(self.block, past_key_values)):
            ...
            if i > 0 and self.use_input_prefix_embeds and not self.is_decoder and self.enable_layer_wise_prefix:
                prefix, real_hidden_states = torch.split(hidden_states, [self.prefix_length,
                                                                         hidden_states.size(1) - self.prefix_length],
                                                         dim=1)
...
class T5ForPT(T5PreTrainedModel):
    ...
    def __init__(self, config, **model_args):
        ...
        self.encoder = T5Stack(encoder_config, self.shared, prefix_length=self.local_config.prefix_length,
        ...
        self.decoder = T5Stack(decoder_config, self.shared, prefix_length=self.local_config.prefix_length,
                               prefix_set_number=self.local_config.prefix_set_number,
                               enable_layer_wise_prefix=self.local_config.enable_layer_wise_prefix)
...
def get_full_pretrain_t5_model(config):
    tokenizer = T5Tokenizer.from_pretrained(config.tokenizer_type)
    model = T5ForPT.from_pretrained(config.lm_type, local_config=config)
...
def get_bert_model(config, output_size):
    ...
    model = BertForTokenClassification.from_pretrained(config.lm_type, num_labels=output_size,
